...
Q The following questions are related creating basic data tables that Danny and his team can use 
 ...
 does not need the ranking for non-member purchases so he expects null ranking values for the 
...
select *,
    case
        ...
        else rank() over(partition by customer_id, member order by order_date) end as ranks
 from cte;